...
RSpec.describe "ApiDevelopments", type: :request do
  ...
  describe "RDBMS-backed" do
    before(:each) { City.where(:name => 'test').delete_all }
    after(:each)  { City.where(:name => 'test').delete_all }
  ...
  describe "MongoDB-backed" do
    before(:each) { State.where(:name => 'test').delete_all }
    after(:each)  { State.where(:name => 'test').delete_all }
